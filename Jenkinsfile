pipeline {
  ...
  environment {
        ...
        NAMESPACE = "@eeacms"
        SONARQUBE_TAGS = "volto.eea.europa.eu,demo-www.eea.europa.eu,circularity.eea.europa.eu,www.eea.europa.eu-en,climate-adapt.eea.europa.eu"
        DEPENDENCIES = ""
        VOLTO = ""
    }
    ...
    stage('Code') {
      ...
      steps {
        parallel(
          ...
          "ES lint": {
            node(label: 'docker') {
              sh '''docker pull plone/volto-addon-ci; docker run -i --rm --name="$BUILD_TAG-eslint" -e NAMESPACE="$NAMESPACE" -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" -e VOLTO=$VOLTO plone/volto-addon-ci eslint'''
            }
          },
